                                ...
                                <div class="col-md-4">
                                    ...
                                    <div id="dialog" style="display:none">
                                        <div class="col-md-8">
                                            ...
                                            <?=form_open_multipart('dialog/upload_dialog', "id='formDialog'"); ?>
                                                <div class="input-group">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $.show_file = function() {
            ...
            $.ajax({
                url     : '<?=base_url()?>dialog/dialog_list_file',
                method  : 'POST',
                data    : { level : level} ,
                async   : true,
                dataType: 'JSON',
                success : function(hasil){
                    var url         = '<?=base_url()?>admin/download_dialog_text';
                    parsedata   = $.parseJSON(hasil.data);
                    $.each(parsedata, function( index, value ) {
                        var file_name = value;
                            url = '<?=base_url()?>dialog/download_dialog_file?level='+level+'&file_name='+file_name;
                            txt1 = $(`<th>`+value+`</th> 
                        				<th>
        ...
        $.delete_data = function(file_name) {
            ...
            $.ajax({
            url     : "<?=base_url()?>dialog/delete_dialog_file",
            method  : "POST",
    ...
    $('#submit_dialog').on("click", function (e) {
        ...
        $.ajax({
            url: "<?=base_url()?>dialog/upload_file_dialog",
            method: "POST",
    ...
    $('#clear_data_content').on("click", function(e) {
        ...
        // console.log(level);
        $.ajax ({
            url         : '<?=base_url()?>dialog/dialog_clear_data_content',
            method      : "POST",
            data        : {level : level},
            async       : true,
            dataType    : 'JSON',
