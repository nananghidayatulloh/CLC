                    ...
                    <div class="body table-responsive">
                        <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                            ...
                            <tbody>
                              <?php
                              $no =1;
                              foreach ($comprehension as $key => $value) : ?>
                                <tr>
                                  <td><?= $no++ ?></td>
                                  <td><?= $value->level ?></td>
                                  <td><?= $value->unit.' '.$value->title ?></td>
                                  <td><?= $value->mode ?></td>
                                  <td><?= $value->product_code ?></td>
                                  <td> <a onclick="return confirm('Yakin ingin menghapus ?')" href="<?= base_url('lesson_plan/hapus_lesson_comprehension/'.$value->id.'/'.$mode) ?>" class="btn-sm btn-danger">Hapus</a> </td>
                                </tr>
                              <?php endforeach; ?>
